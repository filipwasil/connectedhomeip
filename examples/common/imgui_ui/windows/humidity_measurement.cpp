...
void HumidityMeasurement::UpdateState()
{
    ...
    UpdateStateNullable(mEndpointId, mTargetRelativeHumidityMin, mRelativeHumidityMin,
                        &chip::app::Clusters::RelativeHumidityMeasurement::Attributes::MinMeasuredValue::Set,
    ...
    UpdateStateNullable(mEndpointId, mTargetRelativeHumidityMax, mRelativeHumidityMax,
                        &chip::app::Clusters::RelativeHumidityMeasurement::Attributes::MaxMeasuredValue::Set,
...
void HumidityMeasurement::Render()
{
    ...
    if (hValue != uiValue)
    {
        mTargetRelativeHumidityValue.Update(static_cast<uint16_t>(hValue));
    }

    ImGui::LabelText("Relative Humidity Min", "%d", uiMin);
    if (minValue != uiMin)
    {
        mTargetRelativeHumidityMin.Update(static_cast<uint16_t>(minValue));
    }

    ImGui::LabelText("Relative Humidity Max", "%d", uiMax);
    if (maxValue != uiMax)
    {
        mTargetRelativeHumidityMax.Update(static_cast<uint16_t>(maxValue));
    }

    ImGui::LabelText("Relative Humidity Tolerance", "%d", uiTolerance);
    if (tValue != uiTolerance)
    {
        mTargetRelativeHumidityTolerance = tValue;
    }
